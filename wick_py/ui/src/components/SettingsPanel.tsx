...
export function SettingsPanel({
  ...
  // ── Sandbox mode ────────────────────────────────────────────────────
  //   Local Docker  = mode:'local'  → per-user folder inside same container
  //   Remote Docker = mode:'remote' → separate Docker container on remote daemon
  ...
  const handleModeSwitch = useCallback(async (mode: SandboxMode) => {
    ...
    try {
      if (mode === 'local') {
        // Local: per-user folder inside the same container
          ...
          {isSandboxAgent && (
            <section className="settings-section">
              ...
              <span className="settings-hint">
                {sandboxMode === 'local'
                  ? 'Per-user workspace folder inside this container.'
                  : 'Separate Docker container per user on a remote daemon.'}
              </span>
